...
public class ModelTurtle extends ModelQuadruped {
    private final ModelRenderer turtleRenderer;
    public ModelTurtle(float scale) {
        ...
        this.turtleRenderer = new ModelRenderer(this);
        this.turtleRenderer.setTextureOffset(70, 33).addBox(-4.5F, 3.0F, -14.0F, 9, 18, 1, 0.0F);
        this.turtleRenderer.setRotationPoint(0.0F, 11.0F, -10.0F);
        int i = 1;
    ...
    public void render(Entity entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
        ...
        if (this.isChild) {
            ...
            this.leg4.render(scale);
            if (entityturtle.hasEgg()) {
                this.turtleRenderer.render(scale);
            }
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn) {
        ...
        this.turtleRenderer.rotateAngleX = ((float)Math.PI / 2F);
        if (!entityturtle.isInWater() && entityturtle.onGround) {
            float f = entityturtle.getDigging() ? 4.0F : 1.0F;
            float f1 = entityturtle.getDigging() ? 2.0F : 1.0F;
            float f2 = 5.0F;
